...
def synthetic_dataset():
    ...
    y = ((X["transaction_count"] < 10) & (X["total_amount"] < 3000)).astype(int)
    return X, y
...
def test_explain_returns_one_shap_row_per_instance(synthetic_dataset, classifier):
    X, y = synthetic_dataset
    pipeline = Pipeline([("scaler", StandardScaler()), ("classifier", classifier)]).fit(X, y)

    explainer = build_explainer(pipeline, background=X.sample(30, random_state=1))
    sample = X.sample(10, random_state=2)
    result = explain(
        pipeline, explainer, sample, feature_names=list(DEFAULT_CONFIG.feature_columns)
    )

    assert result.shap_values.shape == (10, len(DEFAULT_CONFIG.feature_columns))
...
def test_global_feature_importance_sums_to_ranked_features(synthetic_dataset):
    ...
    explainer = build_explainer(pipeline, background=X.sample(30, random_state=1))
    result = explain(
        pipeline, explainer, X.sample(20, random_state=2),
    ...
    assert (importance["mean_abs_shap"].values[:-1] >= importance["mean_abs_shap"].values[1:]).all()
...
def test_explain_instance_returns_one_row_per_feature(synthetic_dataset):
    ...
    explainer = build_explainer(pipeline, background=X.sample(30, random_state=1))
    result = explain(
        pipeline, explainer, X.sample(5, random_state=2),
        feature_names=list(DEFAULT_CONFIG.feature_columns),
    )
    instance_explanation = explain_instance(result, index=0)

    assert len(instance_explanation) == len(DEFAULT_CONFIG.feature_columns)
    assert {"feature", "feature_value", "shap_value"}.issubset(instance_explanation.columns)
