...
def test_aggregate_customer_features_returns_expected_columns(sample_transactions):
...
def test_aggregate_customer_features_values_are_correct(sample_transactions):
    result = aggregate_customer_features(sample_transactions).set_index("CustomerId")
...
def test_create_rfm_returns_one_row_per_customer(sample_transactions):
    rfm = create_rfm(sample_transactions, snapshot_offset_days=TEST_CONFIG.snapshot_offset_days)
...
def test_label_risk_flags_the_least_engaged_customer_as_high_risk(sample_transactions):
    rfm = create_rfm(sample_transactions)
...
def test_build_training_dataset_merges_features_and_target_on_customer_id(sample_transactions):
