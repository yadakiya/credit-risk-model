...
def render_scoring_tab(model, background: pd.DataFrame):
    ...
    st.write("Enter a customer's transaction summary to get a risk probability and see why.")

    col1, col2 = st.columns([1, 2])

    with col1:
        total_amount = st.number_input(
            "Total transaction amount", min_value=0.0, value=15000.0, step=100.0
        )
        ...
        std_amount = st.number_input(
            "Std. deviation of transaction amount", min_value=0.0, value=320.0, step=10.0
        )
    ...
    if model is None:
        st.error("No trained model found. Run `python -m src.train` first to produce model.pkl.")
        return
    ...
    with col2:
        ...
        st.markdown(f"### {risk_color} {'High risk' if is_high_risk else 'Low risk'}")
        ...
        colors = ["#d62728" if v > 0 else "#2ca02c" for v in contributions["shap_value"]]
        ax.barh(contributions["label"], contributions["shap_value"], color=colors)
        ax.axvline(0, color="black", linewidth=0.8)
        ax.set_xlabel("Contribution to risk score (red = increases risk, green = decreases risk)")
        ax.invert_yaxis()
        st.pyplot(fig)
...
def render_portfolio_tab(model, background: pd.DataFrame):
    ...
    if dataset is not None and "is_high_risk" in dataset.columns:
        ...
        risk_counts = dataset["is_high_risk"].value_counts().rename({0: "Low risk", 1: "High risk"})
...
def main():
    ...
    if dataset is not None:
        background = dataset[FEATURES].sample(min(len(dataset), 50), random_state=42)
    else:
        background = pd.DataFrame([{
            "total_amount": 5000.0, "avg_amount": 300.0,
            "transaction_count": 15, "std_amount": 150.0,
        }] * 10)
